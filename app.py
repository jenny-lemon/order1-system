...
<style>
    ...
    .page-title { font-size: 2.5rem; font-weight: 800; color: #1f2937; margin-bottom: 0.2rem; }
    .page-subtitle { color: #6b7280; margin-bottom: 1.5rem; font-size: 1rem; }
    .card { background: white; border-radius: 18px; padding: 22px 22px 8px 22px; box-shadow: 0 6px 18px rgba(15, 23, 42, 0.06); border: 1px solid #eef2f7; margin-bottom: 18px; }
    .section-title { font-size: 1.05rem; font-weight: 700; color: #111827; margin-bottom: 0.8rem; }
    .hint-box { background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 12px; padding: 12px 14px; color: #4b5563; font-size: 0.95rem; margin-bottom: 10px; }
    .stButton > button { background: linear-gradient(135deg, #4f46e5, #6366f1); color: white; border: none; border-radius: 10px; font-weight: 700; height: 44px; padding: 0 20px; }
    .stButton > button:hover { opacity: 0.95; }
    div[data-testid="stMetric"] { background: white; border: 1px solid #eef2f7; border-radius: 14px; padding: 8px 14px; box-shadow: 0 4px 12px rgba(15, 23, 42, 0.04); }
    .action-note { font-size: 0.92rem; color: #6b7280; margin-top: -6px; margin-bottom: 10px; }
    .fail-box { background: #fff7f7; border: 1px solid #fecaca; border-radius: 12px; padding: 12px 14px; margin-bottom: 10px; }
    .ok-box { background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 12px; padding: 12px 14px; margin-bottom: 10px; }
</style>
...
st.markdown('<div class="page-subtitle">支援建單、寄確認信、改 Google 日曆，並可指定列號批次處理。</div>', unsafe_allow_html=True)
...
with st.container():
    ...
    st.markdown('<div class="hint-box">列號支援單列、逗號分隔、多段區間，例如：<b>2</b>、<b>2,3,5</b>、<b>2,3,5-7</b></div>', unsafe_allow_html=True)
    st.markdown('<div class="section-title">🧩 執行項目</div>', unsafe_allow_html=True)
    selected_actions = st.multiselect("請勾選要執行的項目", options=["建單", "寄確認信", "改 Google 日曆"], default=["建單", "寄確認信", "改 Google 日曆"])
    st.markdown('<div class="action-note">可自由組合，例如只寄確認信、只改日曆，或全流程一起跑。</div>', unsafe_allow_html=True)
...
if run_clicked:
    ...
    for row_no in target_rows:
        ...
        try:
            ...
            if "selected_actions" in str(e):
                ...
                st.stop()
            total_fail += 1; total_processed += 1
            failed_records.append({"row": row_no, "name": "未知", "error": str(e)})
    ...
    m1.metric("成功", total_success); m2.metric("失敗", total_fail); m3.metric("總處理", total_processed)
    if total_fail == 0:
        st.success("🎉 執行完成")
    else:
    ...
    if failed_records:
        ...
        for item in failed_records:
            ...
            st.write(f"**第 {item.get('row', '')} 列｜{item.get('name', '未知')}**")
            if item.get("error", ""):
