...
def process_one_group(session, rows_with_idx, token, gcal_service, region, backend_user_id=None, selected_actions=None):
    ...
    addr_check = check_contain(session, member.get("member_id", ""), target_address, best_addr.get("lat", ""), best_addr.get("lng", ""), token, clean_type_id)
    ...
    base_data = prepare_base_order_data(row0, member_payload, best_addr, clean_type_id, people, hours, system_period, mapped)
    calc_date = get_date_str(row0["日期"])
    calc_data = base_data.copy(); calc_data["date_s"] = calc_date
    if not calculate_hour(session, calc_data, token):
        raise Exception("計算時數失敗")
    row_details = []
    for row_num, row in rows_with_idx:
        date_s = get_date_str(row["日期"])
        row_details.append({"row_num": row_num,"date": date_s,"slot": f"{date_s}_{system_period}","price": calc_occurrence_price(row["日期"], people, hours),"display_period": system_display_period,"row": row})
    ...
    if not need_create_order:
        for detail in row_details:
            ...
            stage_result = {"訂單編號": existing_order_no,"結果": "成功" if existing_order_no else "失敗","原因": "" if existing_order_no else "無訂單編號，無法寄信或改日曆","沒班表日期": "","餘額不足未送": "","簡訊實際服務時間": base_data.get("period", ""),"客人備註": base_data.get("memo", ""),"確認信": "","日曆改色結果": "","日曆改色原因": "","日曆原色": "","日曆新色": ""}
            if existing_order_no and has_action(selected_actions, "寄確認信"):
                stage_result.update(stage_send_confirmation(existing_order_no, session))
            if has_action(selected_actions, "改 Google 日曆"):
                calendar_info = stage_calendar_color(detail["row"], gcal_service, region)
                stage_result.update(calendar_info)
                if existing_order_no:
                    stage_result.update(stage_update_status(existing_order_no, calendar_info))
    ...
    if not valid_details:
        for detail in row_details:
            row_results[detail["row_num"]] = {"訂單編號": "","結果": "失敗","原因": "無班表","沒班表日期": detail["date"],"餘額不足未送": "","簡訊實際服務時間": base_data.get("period", ""),"客人備註": base_data.get("memo", ""),"確認信": "","日曆改色結果": "","日曆改色原因": "","日曆原色": "","日曆新色": ""}
        return row_results
    ...
    insufficient_dates, send_details = [], []
    ...
    for detail in row_details:
        if detail["date"] in no_slot_dates:
            ...
            row_results[detail["row_num"]] = {"訂單編號": "","結果": "未送","原因": "餘額不足","沒班表日期": "","餘額不足未送": detail["date"],"簡訊實際服務時間": base_data.get("period", ""),"客人備註": base_data.get("memo", ""),"確認信": "","日曆改色結果": "","日曆改色原因": "","日曆原色": "","日曆新色": ""}
    if not send_details:
        return row_results
    ...
    for price, details in batches.items():
        payload = base_data.copy(); payload["price"] = str(price); payload["price_vvip"] = "0"; payload["fare"] = "0"
        slots = [d["slot"] for d in details]
        session.post(BOOKING_URL, data={**payload, "_token": token, "date_list[]": slots}, headers=HEADERS, allow_redirects=True)
        time.sleep(1)
        for detail in details:
            order_no = fetch_order_no_by_date_and_period(session, detail["date"], detail["display_period"])
            if not order_no:
                row_results[detail["row_num"]] = {"訂單編號": "","結果": "已送出","原因": "抓不到訂單編號","沒班表日期": "","餘額不足未送": "","簡訊實際服務時間": base_data.get("period", ""),"客人備註": base_data.get("memo", ""),"確認信": "","日曆改色結果": "","日曆改色原因": "","日曆原色": "","日曆新色": ""}
                continue
            stage_result = {"訂單編號": order_no,"結果": "成功","原因": "","沒班表日期": "","餘額不足未送": "","簡訊實際服務時間": base_data.get("period", ""),"客人備註": base_data.get("memo", ""),"確認信": "","日曆改色結果": "","日曆改色原因": "","日曆原色": "","日曆新色": ""}
            if has_action(selected_actions, "寄確認信"):
